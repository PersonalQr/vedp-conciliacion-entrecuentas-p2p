# Orq 2 conciliacion entrecuentas p2p

Es una herramienta que genera los cruces para la concilacin y compesacin del proceso de transaccin con llaves esto para el tema de entrecuentas y p2p
...
```
pip install vedp-conciliacion-entrecuentas-p2p
```
*Si no se tiene configurado Artifactory:*

```
pip install vedp-conciliacion-entrecuentas-p2p -i https://artifactory.apps.bancolombia.com/api/pypi/pypi-bancolombia/simple --trusted-host artifactory.apps.bancolombia.com
```
...
```
python -m vedp_conciliacion_entrecuentas_p2p.ejecucion
```
...
*Logs de estabilidad:*
```
python -m vedp_conciliacion_entrecuentas_p2p.ejecucion -lt "est"
```

*Logs de compilación:*
```
python -m vedp_conciliacion_entrecuentas_p2p.ejecucion -lt "cmp" -pl [porcentaje]
```
...
Adicionalmente, con el fin de abreviar los comandos de ejecución, se habilitó utilizar el nombre del paquete (con guiones bajos) directamente para reemplazar la sintáxis de módulos de python; lo cuál permite sustituir la expresión ```python -m vedp_conciliacion_entrecuentas_p2p.ejecucion``` por ```vedp_conciliacion_entrecuentas_p2p``` en cada comando de ejecución si así lo desea.
