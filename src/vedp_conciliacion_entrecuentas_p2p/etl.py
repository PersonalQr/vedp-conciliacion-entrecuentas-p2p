...
class ExtractTransformLoad(Step):
    ...
    def enviar_correo(self):
        ...
        df_offus["suma_total"] = df_offus["suma_total"].apply(
            lambda x: f"${'{:,}'.format(int(x)).replace(',', '.')}" if pd.notnull(x) and str(x).isdigit() else x
        )
    ...
    def compensar_pmd(self):
        ...
        df_pmd["suma_total"] = df_pmd["suma_total"].apply(
            lambda x: f"${'{:,}'.format(int(x)).replace(',', '.')}" if pd.notnull(x) and str(x).lstrip('-').isdigit() else x
        )
        # Depuración
        ...
        # Crear datos extra
        extra_data = [
            {"descripcion": "Total Compensar Transaccional", "suma_total": f"${total_suma:,}".replace(",", ".")},
            {"descripcion": "PMD P2P REDEBAN", "suma_total": f"${pmd_neto:,}".replace(",", ".")},
            {"descripcion": "Comparacion", "suma_total": f"${total_suma - pmd_neto:,}".replace(",", ".")},
        ]
