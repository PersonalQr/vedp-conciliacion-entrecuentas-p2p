...
-- Descripción: Creación y llenado de la tabla temporal `temp_hub_pos_data_offus`.
--              Esta tabla almacena información combinada de transacciones
--              de HUB, POS y depósitos, enfocándose en el tipo de transacción
--              OFFUS y generando indicadores adicionales.
-- -----------------------------------------------------------------------------

-- Eliminación de la tabla temporal `temp_hub_pos_data_offus` si ya existe
-- Esto asegura que la tabla no contenga datos antiguos antes de ser recreada.
DROP TABLE IF EXISTS {zonap}.temp_hub_pos_data_offus PURGE;

-- Creación y llenado de la tabla temporal `temp_hub_pos_data_offus`
...
-- agregados como estados, montos y conteos para transacciones OFFUS.
CREATE TABLE {zonap}.temp_hub_pos_data_offus AS
SELECT
    tem_hub.tipotrx,
    tem_hub.nattrx,
    tem_hub.nrorastreo,
